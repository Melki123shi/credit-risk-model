...
class TransactionInput(BaseModel):
    ...
    subscription_id: str = Field(..., description="Customer subscription identifier")
    customer_id: str = Field(..., description="Customer unique identifier")
    provider_id: str = Field(..., description="Source provider of the item")
    product_id: str = Field(..., description="Item being bought")
    product_category: str = Field(..., description="Product category")
    channel_id: str = Field(..., description="Customer channel")
    amount: float = Field(..., description="Transaction amount")
    value: int = Field(..., description="Absolute value of the amount")
...
class PredictionResponse(BaseModel):
    ...
    recommended_loan_amount: Optional[float] = Field(
        None, description="Recommended max loan amount"
    )
    recommended_loan_duration_months: Optional[int] = Field(
        None, description="Recommended loan duration in months"
    )
